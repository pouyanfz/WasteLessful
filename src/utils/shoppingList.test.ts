...
describe('shouldAutoAdd', () => {
  ...
  it('returns false when item is expiring soon but autoAddOnExpiry is false', () => {
    expect(
      shouldAutoAdd({ ...base, expiresAt: daysFromToday(2), autoAddOnExpiry: false }),
    ).toBe(false)
  })

  it('returns false when item is not expiring soon', () => {
    expect(shouldAutoAdd({ ...base, expiresAt: daysFromToday(10) })).toBe(false)
  })
  ...
  it('returns true when quantity is low and autoAddOnLowQuantity is true', () => {
    expect(shouldAutoAdd({ ...base, expiresAt: null, current: 100, initial: 500 })).toBe(true) // 20% ≤ 25%
  })
