...
function TabStrip({
  ...
  function cancel() { setAdding(false); setInput(""); }

  function tabCls(active: boolean) {
    return `shrink-0 py-3 px-4 text-sm font-medium border-b-2 transition-colors whitespace-nowrap ${
      active ? "border-green-500 text-green-600" : "border-transparent text-gray-500 hover:text-gray-700"
    }`;
  }
      ...
      <div className="flex items-center max-w-lg mx-auto">
        ...
        <div className="relative flex-1 min-w-0">
          <div ref={scrollRef} className="flex overflow-x-auto scrollbar-hide">
            <button onClick={() => onChange("all")} className={tabCls(activeTab === "all")}>All</button>

            {/* Custom shopping lists */}
            {lists.map((l) => (
              <button key={l.id} onClick={() => onChange(l.id)} className={tabCls(activeTab === l.id)}>
                {l.name}
              </button>
            ))}
            ...
            {groups.map((g) => (
              <button key={g.id} onClick={() => onChange(g.id)} className={tabCls(activeTab === g.id)}>
                {g.name}
              </button>
            ))}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-800">Manage Lists</h2>
        ...
        {lists.length > 0 && (
          <>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-5 pt-3 pb-1">Your lists</p>
              ...
              {lists.map((l, i) => (
                <li key={l.id} className="flex items-center gap-3 px-5 py-3">
                  <div className="flex flex-col gap-0.5">
                    ...
                    <button onClick={() => move(i, 1)} disabled={i === lists.length - 1}
                      className="w-6 h-5 flex items-center justify-center text-gray-400 hover:text-gray-700 disabled:opacity-20 transition-colors">
                  ...
                  <button onClick={() => onDelete(l.id)} className="text-sm text-red-400 hover:text-red-600 px-2 py-1 rounded hover:bg-red-50 transition-colors">
                    Delete
                  </button>
                </li>
              ))}
        ...
        {groups.length > 0 && (
          <>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide px-5 pt-3 pb-1">Group tabs</p>
            <ul className="divide-y divide-gray-100">
              {groups.map((g) => (
                <li key={g.id} className="flex items-center gap-3 px-5 py-3">
                  <span className="flex-1 text-sm text-gray-500 truncate">{g.name}</span>
                  <span className="text-xs text-gray-400">Managed in Items</span>
                </li>
              ))}
            </ul>
          </>
        ...
        {lists.length === 0 && groups.length === 0 && (
          <p className="text-center text-sm text-gray-400 py-8">No lists yet.</p>
        )}

        <div className="px-5 py-4 border-t border-gray-100">
          <button onClick={onClose} className="w-full py-2.5 rounded-xl bg-green-500 text-white text-sm font-medium hover:bg-green-600 transition-colors">
            Done
          </button>
        </div>
      </div>
...
function EditShoppingItemSheet({
  ...
  const [unit, setUnit] = useState(UNITS.includes(item.quantity.unit) ? item.quantity.unit : "pack");
  const [customUnit, setCustomUnit] = useState(!UNITS.includes(item.quantity.unit) ? item.quantity.unit : "");
  const [isCustomUnit, setIsCustomUnit] = useState(!UNITS.includes(item.quantity.unit));
  ...
  function handleSave(e: React.FormEvent) {
    ...
    onSave({ ...item, name: name.trim(), quantity: { amount, unit: resolvedUnit }, updatedAt: now });
    onClose();
  }

  const listLabel =
    item.shoppingListId ? lists.find((l) => l.id === item.shoppingListId)?.name :
    item.groupId ? groups.find((g) => g.id === item.groupId)?.name :
    null;

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl p-6 flex flex-col gap-5 max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Edit Item</h2>
            {listLabel && <p className="text-xs text-gray-400 mt-0.5">in {listLabel}</p>}
          </div>
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex gap-2">
              <input type="number" min={0.1} step="any" value={amount} onChange={(e) => setAmount(Number(e.target.value))}
              ...
              <select
                ...
                onChange={(e) => { if (e.target.value === "__custom__") setIsCustomUnit(true); else { setIsCustomUnit(false); setUnit(e.target.value); } }}
            ...
            {isCustomUnit && (
              <input type="text" value={customUnit} onChange={(e) => setCustomUnit(e.target.value)} placeholder="e.g. jar, tray…" autoFocus
          ...
          {item.autoAdded && item.linkedItemName && item.linkedItemName !== item.name && (
            <p className="text-xs text-green-700 bg-green-50 rounded-lg px-3 py-2">
              Auto-added to restock: <span className="font-medium">{item.linkedItemName}</span>
            </p>
          )}
        ...
        {isBought && (
          item.addedToInventory ? (
            ...
            <button onClick={() => onAddToInventory(item)}
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl p-6 flex flex-col gap-4" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-gray-900">Mark as bought?</h2>
            <p className="text-sm text-gray-500 mt-1">
              Do you want to add <span className="font-medium text-gray-700">{item.name}</span> to your inventory too?
            </p>
          </div>
...
function ShoppingRow({
  item, groupLabel, groupColor, recipeLabel, onToggle, onEdit,
}: {
  item: ShoppingItem;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className={`text-sm font-medium ${bought ? "line-through text-gray-400" : "text-gray-900"}`}>
          {item.name}
        </p>
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-xs text-gray-400">{item.quantity.amount} {item.quantity.unit}</span>
          {item.autoAdded && (
            <span className="text-[10px] bg-green-100 text-green-700 rounded-full px-1.5 py-0.5 font-medium">
              {item.linkedItemName && item.linkedItemName !== item.name ? `restocking ${item.linkedItemName}` : "auto"}
            </span>
          )}
          {recipeLabel && (
            <span className="text-[10px] bg-purple-50 text-purple-600 rounded-full px-1.5 py-0.5 font-medium">for {recipeLabel}</span>
          )}
          {groupLabel && (
            <span
              style={groupColor ? { backgroundColor: groupBadgeBg(groupColor), color: groupColor } : {}}
      ...
      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#d1d5db" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="shrink-0">
...
export default function ShoppingListPage() {
  ...
  const { groups, shoppingItems: items, setShoppingItems: setItems, shoppingLists: lists, setShoppingLists: setLists, recipes } = useAppData();
  const [activeTab, setActiveTab] = useState("all");
  const [showManage, setShowManage] = useState(false);
  const [pendingToggleItem, setPendingToggleItem] = useState<ShoppingItem | null>(null);
  ...
  const scopedItems =
    activeTab === "all" ? items :
    isGroupTab ? items.filter((i) => i.groupId === activeTab) :
    isListTab ? items.filter((i) => i.shoppingListId === activeTab) :
    items;
  ...
  function getRowLabel(item: ShoppingItem): string | null {
    ...
    if (item.shoppingListId) return lists.find((l) => l.id === item.shoppingListId)?.name ?? null;
    if (item.groupId) return groups.find((g) => g.id === item.groupId)?.name ?? null;
    return null;
  }
  ...
  function toggleStatus(id: string) {
    ...
    if (item.status === "toBuy") {
      ...
      setItems((prev) => prev.map((i) =>
        i.id === id ? { ...i, status: "toBuy" as const, boughtBy: null, boughtAt: null, updatedAt: now } : i
      ));
    }
  }
  ...
  function markBought(id: string) {
    const now = makeTimestamp(new Date()) as never;
    setItems((prev) => prev.map((i) =>
      i.id === id ? { ...i, status: "bought" as const, boughtBy: "user-1", boughtAt: now, updatedAt: now } : i
    ));
  }
  ...
  function markBoughtAndAddedToInventory(id: string) {
    const now = makeTimestamp(new Date()) as never;
    setItems((prev) => prev.map((i) =>
      i.id === id ? { ...i, status: "bought" as const, boughtBy: "user-1", boughtAt: now, updatedAt: now, addedToInventory: true } : i
    ));
  }

  function deleteItem(id: string) { setItems((prev) => prev.filter((i) => i.id !== id)); }
  function saveItem(updated: ShoppingItem) { setItems((prev) => prev.map((i) => i.id === updated.id ? updated : i)); }
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    const resolvedUnit = isCustomAddUnit ? (customAddUnit.trim() || newUnit) : newUnit;
    const now = makeTimestamp(new Date()) as never;
    const newItem: ShoppingItem = {
      ...
      linkedRecipeId: null, linkedItemId: null, linkedItemName: null,
      status: "toBuy", autoAdded: false, addedBy: "user-1",
      boughtBy: null, addedAt: now, boughtAt: null, updatedAt: now,
    };
    setItems((prev) => [newItem, ...prev]);
    setNewName(""); setNewAmount(1); setNewUnit("pack");
    setIsCustomAddUnit(false); setCustomAddUnit("");
  }

  function handleAddToInventory(item: ShoppingItem) {
    setEditingItem(null);
    setPendingToggleItem(null);
    setItems((prev) => prev.map((i) => i.id === item.id ? { ...i, addedToInventory: true } : i));
    navigate("/", {
      state: {
        prefillItem: { name: item.name, amount: item.quantity.amount, unit: item.quantity.unit, groupId: item.groupId },
      },
    });
  }
  ...
  const [addDestTab, setAddDestTab] = useState<string>(lists[0]?.id ?? groups[0]?.id ?? "all");
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      {/* Add form */}
      <form onSubmit={showDestSelector ? handleAddFromAll : handleAdd} className="bg-white border-b border-gray-100 px-4 py-3 flex flex-col gap-2">
        {/* Destination selector — only on All tab */}
        {showDestSelector && (lists.length > 0 || groups.length > 0) && (
          <div className="flex gap-1.5 overflow-x-auto scrollbar-hide">
            {lists.map((l) => (
              <button key={l.id} type="button" onClick={() => setAddDestTab(l.id)}
            ...
            {lists.length > 0 && groups.length > 0 && <div className="w-px bg-gray-200 my-1 shrink-0" />}
            {groups.map((g) => (
              <button key={g.id} type="button" onClick={() => setAddDestTab(g.id)}
        ...
        <div className="flex gap-2">
          <input type="text" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Add item…"
          ...
          <input type="number" min={0.1} step="any" value={newAmount} onChange={(e) => setNewAmount(Number(e.target.value))}
          ...
          <select value={isCustomAddUnit ? "__custom__" : newUnit}
            onChange={(e) => { if (e.target.value === "__custom__") setIsCustomAddUnit(true); else { setIsCustomAddUnit(false); setNewUnit(e.target.value); } }}
          ...
          <button type="submit" disabled={!newName.trim() || (isCustomAddUnit && !customAddUnit.trim())}
            className="w-10 h-10 rounded-full bg-green-500 text-white text-xl flex items-center justify-center shadow disabled:opacity-40 shrink-0">+</button>
        </div>
        {isCustomAddUnit && (
          <input type="text" value={customAddUnit} onChange={(e) => setCustomAddUnit(e.target.value)} placeholder="e.g. jar, tray…" autoFocus
            className="border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-400" />
        )}
      </form>
      ...
      <main className="px-4 py-4 max-w-lg mx-auto flex flex-col gap-6">
        {toBuy.length > 0 && (
          <section className="flex flex-col gap-2">
            <h2 className="text-xs font-semibold uppercase tracking-wide text-gray-400 px-1">To buy · {toBuy.length}</h2>
            {toBuy.map((item) => (
              <ShoppingRow key={item.id} item={item} groupLabel={getRowLabel(item)} groupColor={item.groupId ? (groups.find((g) => g.id === item.groupId)?.color ?? null) : null} recipeLabel={item.linkedRecipeId ? (recipes.find((r) => r.id === item.linkedRecipeId)?.name ?? null) : null} onToggle={toggleStatus} onEdit={setEditingItem} />
            ))}
          </section>
        )}
        ...
        {bought.length > 0 && (
          <section className="flex flex-col gap-2">
            <div className="flex items-center justify-between px-1">
              <h2 className="text-xs font-semibold uppercase tracking-wide text-gray-400">Bought · {bought.length}</h2>
              <button onClick={clearBought} className="text-xs text-red-400 hover:text-red-600">Clear</button>
            </div>
            {bought.map((item) => (
              <ShoppingRow key={item.id} item={item} groupLabel={getRowLabel(item)} groupColor={item.groupId ? (groups.find((g) => g.id === item.groupId)?.color ?? null) : null} recipeLabel={item.linkedRecipeId ? (recipes.find((r) => r.id === item.linkedRecipeId)?.name ?? null) : null} onToggle={toggleStatus} onEdit={setEditingItem} />
            ))}
      ...
      {showManage && (
        <ManageListsModal lists={lists} groups={groups} onReorder={setLists} onDelete={deleteList} onClose={() => setShowManage(false)} />
      )}
      ...
      {editingItem && (
        <EditShoppingItemSheet
          item={editingItem} lists={lists} groups={groups}
